...
function help() {
  ...
  echo ''
  echo 'Commands:'
  echo '   setup               clone and build for the first time'
  echo '   clone               clone repositories in ./customfeeds dir'
  echo '   build               execute docker build script'
  echo '   flash [file [-n]]   flash a built OpenWrt binary to device over ssh'
  echo '   deploy              build and flash'
